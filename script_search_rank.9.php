	...
	while($row=mysqli_fetch_array($table_result)){
		...
		for($index=0;$index<$num_fields;$index++){
			...
			if(strlen($row[$index])>4){
				...
				$r = log(strlen($description)/10);
				$r = floor(100*$r);
				$escaped_r = (int) $r;
				$sql="INSERT INTO `search_rank` (`r`,`table_id`,`record_id`,`field_id`,`field_name`,`table_name`,`description`) VALUES ('".$escaped_r."','".$escaped_table_id."','".$escaped_record_id."','".$index."','".$escaped_field_name[$index]."','".$escaped_table_name."','".$escaped_cropped_description."')";
				mysqli_query($db, $sql);
				$result_id_row = mysqli_fetch_array(mysqli_query($db, "SELECT `result_id` FROM `search_rank` WHERE `table_id`='$escaped_table_id' AND `record_id`='$escaped_record_id' AND `field_id`='$index'"));
				...
				//The term derivative is used here loosely
				for($derivative=0;$derivative<$num_keywords&&$derivative<10;$derivative++){
