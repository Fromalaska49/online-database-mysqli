	...
	while($row=mysqli_fetch_array($table_result)){
		...
		for($index=0;$index<$num_fields;$index++){
			...
			if(strlen($row[$index])>6){
				...
				$r = floor(100*$r);
				$escaped_r = (int) $r;
				$sql="INSERT INTO `search_rank` (`r`,`table_id`,`record`,`field`,`field_name`,`company_name`,`description`) VALUES ('".$escaped_r."','".$escaped_keyword."','".$escaped_table_id."','".$escaped_record_id."','".$index."','".$escaped_field_name[$index]."','".$escaped_table_name."','".$escaped_cropped_description."')";
				mysqli_query($db, $sql);
				$result_id_row = mysqli_fetch_array(mysqli_query($db, "SELECT `result_id` FROM `search_rank` WHERE `table_name`='escaped_table_id' AND `record`='$escaped_record_id' AND `record`='$index'"));
				...
				for($differential=0;$differential<$num_keywords;$differential++){
					for($i=0;$i<$num_keywords-$differential;$i++){
						$sanitized_keyword;
						for($delta_i=0;$delta_i<=$differential;$delta_i++){
							if($differential!=0||$i!=0){
								$sanitized_keyword.=$keyword[$i+$delta_i];
